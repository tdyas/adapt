...
But to illustrate most of these concepts, we'll need a sample app that's got a little complexity.
...
The reason `connectEnv` is called via the `useMethod` hook instead of a direct call of the component method is due to `connectEnv` using state to store the hostname or IP address information, once Postgres has been deployed. For more info on how this works, see [Element State](#element-state).
...
## Hooks
Hooks are a concept common to both React and Adapt.
